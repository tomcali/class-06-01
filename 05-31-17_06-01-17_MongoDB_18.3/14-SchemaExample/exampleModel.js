...
var ExampleSchema = new Schema({
  ...
  string: {
    type: String,
    trim: true,
    required: "String is Required"  // this is understood to be set as true
  },
  // This must be a unique number in the collection, and it must be entered
  number: {
    type: Number,  // serves as an ID
    unique: true,
    required: true
  },
  ...
  boolean: Boolean,  // boolean is not a good name for a property
  // array must be an array
  array: Array,
  // date must be a date. The default is the current date
  date: {
    type: Date,
    default: Date.now  // milliseconds since Jan 1 1970 reformatted by JavaScript
  },
